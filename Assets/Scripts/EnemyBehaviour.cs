...
{
    Waiting, // has a "Boxing" sub state
    Moving,
    Attacking
}
...
public class EnemyBehaviour : MonoBehaviour {
...
#if DEBUG
        Assert.IsNotNull(_animator, "DEPENDENCY ERROR: EnemyBehaviour needs an AnimatorComponent");
#endif
    }
    ...
    private void FixedUpdate()
    {
        ...
        transform.GetComponent<Rigidbody>().AddForce(direction * moveSpeed * mass); // F=m*a <=> F=mass*Speed
